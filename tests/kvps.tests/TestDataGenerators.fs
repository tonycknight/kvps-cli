...
type AlphanumericStrings =
  static member Generate() : Arbitrary<string> =
    let isValid (s: string) =
      (String.IsNullOrEmpty s |> not) && s |> Seq.forall Char.IsLetterOrDigit

    ArbMap.defaults |> ArbMap.generate<string> |> Gen.filter isValid |> Arb.fromGen

type Passwords =
  static member Generate() : Arbitrary<Password> =
    let isValid (s: string) = s.Length <= 16

    AlphanumericStrings.Generate().Generator
    |> Gen.filter isValid
...
type KeyValueArbitrary =
  static member Generate() : Arbitrary<KeyValue> =
    let names =
      AlphanumericStrings.Generate().Generator |> Gen.map (fun s -> $"name-{s}")

    let values =
      AlphanumericStrings.Generate().Generator |> Gen.map (fun s -> $"value-{s}")
