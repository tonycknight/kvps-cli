...
module ReflectionTests =
  ...
  let ``getAttrs returns attributes`` () =
    let attrs = Reflection.getAsm () |> Reflection.getAttrs |> Array.ofSeq
  ...
  let ``getCopyrightValue returns non-empty`` () =
    let attr =
      Reflection.getAsm () |> Reflection.getAttrs |> Reflection.getCopyrightValue
  ...
  let ``getVersionValue returns non-empty`` () =
    let attr = Reflection.getAsm () |> Reflection.getAttrs |> Reflection.getVersionValue
