...
module NugetClient=
    let getLatestVersion()=
        try
            ...
            let sourceRepository = Repository.Factory.GetCoreV3(new PackageSource(NuGetConstants.V3FeedUrl))

            let mdr = sourceRepository.GetResource<MetadataResource>()
        
            let vsn = mdr.GetLatestVersion("kvps-cli", false, false, cache, logger, cnxToken).Result
            if vsn.IsPrerelease then
                new System.Version()
            else
                new System.Version(vsn.Major, vsn.Minor, vsn.Patch)
        with
            | _ -> new System.Version()